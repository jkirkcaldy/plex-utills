    ...
    def run_script(): 
        ...
        def check_for_new_poster(tmp_poster):
            ...
            if r:
                try:
                    ablob = r[0].poster
                    poster_file = '/tmp/'+title+'_tmp.png'
                    convert_data(ablob, poster_file)
                    try:
                    ...
                    if poster_hash - bak_poster_hash > cutoff:
                        ...
                        pblob = backup_poster(tmp_poster)
                        dbposter = film_table.query.get(row)
                        dbposter.poster = pblob
                        db.session.commit()
                    else:
...
def tv4kposter():
    ...
    if config[0].tv4kposters == 1 and config[0].tvlibrary != 'None':
    ...
    def run_script():
        def main():
            ...
            def insert_intoTable(hdr, audio, tmp_poster):
                ...
                pblob = backup_poster(tmp_poster)
                if not r:
                    film = film_table(title=title, guid=guid, guids=guids, size=size, res=res, hdr=hdr, audio=audio, poster=pblob)
                    db.session.add(film)   
                    db.session.commit()
                elif r[0].size != size:
                    updateTable(hdr, audio, tmp_poster)
