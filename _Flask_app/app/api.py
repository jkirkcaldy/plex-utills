...
def get_library(lib, plexserver):
    ...
    if n <= 2:
        ...
        films = plexserver.library.section(lib[0])
        return films
...
def upload_tmdb_posters(var=''):
    ...
    film_lib = config[0].filmslibrary.split(',')
    guid = var.split('&')
    guid = guid[1]
    uploaded_poster=[]
    def process_film(library):
        for i in library.search(guid=guid):
            poster = get_poster(i, film_table)
            poster = '/static/img/tmp/'+poster[1]
            uploaded_poster.append(poster)
    def process_tv(library, libtype, table):
        for i in library.search(guid=guid, libtype=libtype):
            poster = get_poster(i, table)
            poster = '/static/img/tmp/'+poster[1]
            uploaded_poster.append(poster)
    ...
    if 'movie' in guid:
        ...
        if n <= 2:
            for l in range(n):
                print('getting plex poster')
                library = plexserver.library.section(film_lib[l])
                process_film(library)
        else:
            process_film(film_lib[0])
    if 'show' in guid:
        scripts.upload_tmdb_show(app, var)
        n = len(tv_lib)
        if n <= 2:
            for l in range(n):
                library = plexserver.library.section(tv_lib[l])
                process_tv(library, 'show', '')
        else:
            process_tv(tv_lib[0], 'show', '') 
    if 'season' in guid:
        ...
        if n <= 2:
            for l in range(n):
                library = plexserver.library.section(tv_lib[l])
                process_tv(library, 'season', season_table)
        else:
            process_tv(tv_lib[0], 'season', season_table)       
    if 'episode' in guid:
        ...
        if n <= 2:
            for l in range(n):
                library = plexserver.library.section(tv_lib[l])
                process_tv(library, 'episode', ep_table)
        else:
            process_tv(tv_lib[0], 'episode', ep_table)         
    return render_template('result.html', pagetitle='Upload', version=version, poster=uploaded_poster[0])
...
def api_process(var=''):
    ...
    if 'movie' in var:
        ...
        if n <= 2:
            for l in range(n):
                library = plexserver.library.section(film_lib[l])
                poster = process_film(library)
        else:
            library = plexserver.library.section(film_lib)
            poster = process_film(library)
        ...
        if n <= 2:
            for l in range(n):
                library = plexserver.library.section(tv_lib[l])
                if 'show' in var:
                    poster = process_tv(library, 'show', '')
                if 'season' in var:
                    poster = process_tv(library, 'season', season_table)
                if 'episode' in var:
                    poster = process_tv(library, 'episode', ep_table)
        else:
            library = plexserver.library.section(tv_lib[0])
            if 'show' in var:
                poster = process_tv(library, 'show', '')
            if 'season' in var:
                poster = process_tv(library, 'season', season_table)
...
@app.route('/api/find_broken_posters_films')
def find_broken_films():
    broken = scripts.find_broken_posters('film')
    return render_template('result.html', broken_posters = broken, version=version)

@app.route('/api/find_broken_posters_shows')
def find_broken_shows():
    broken = scripts.find_broken_posters('show')
    return render_template('result.html', broken_posters = broken, version=version)

@app.route('/api/find_broken_posters_seasons')
def find_broken_seasons():
    broken = scripts.find_broken_posters('season')
    return render_template('result.html', broken_posters = broken, version=version)

@app.route('/api/find_broken_posters_episodes')
def find_broken_episodes():
    broken = scripts.find_broken_posters('episode')
    return render_template('result.html', broken_posters = broken, version=version)

@app.route('/find_broken_posters')
def broken_posters():

    return render_template('broken.html', pagetitle='Find Broken Posters', version=version)