...
def scan_files(config, i, plex): 
    ...
    try:
        ...
        elif any(h in i.hdr for h in versions):
            hdr_version = i.hdr
        else:
            hdr_version = 'none'
        audio = "unknown"
...
def insert_intoTable(guid, guids, size, res, hdr, audio, tmp_poster, banners, title, config, table, db, r, i, b_dir, g, blurred, episode, season):
    ...
    if ('film_table' in str(table) or 'season_table' in str(table)):
        film = table(title=title, guid=guid, guids=str(guids), size=size, res=res, hdr=hdr, audio=audio, poster=b_file, checked='0', url=url)
...
def get_item(library, title, guid):
...
def poster_dance(tmp_poster):
    logger.debug('poster dance')
    tmp_poster = tmp_poster
    poster = cv2.imread(tmp_poster)
    poster = cv2.cvtColor(tmp_poster, cv2.COLOR_BGR2RGB)
    poster = Image.fromarray(tmp_poster)
    poster.save(tmp_poster)